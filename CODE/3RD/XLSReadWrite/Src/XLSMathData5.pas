...
function ErfImp_1(z: double; invert: boolean): double;
...
function EvaluateOddPolynomial11(poly: array of double; z: double): double;
begin
   Result := poly[0] + z * EvaluatePolynomial10(poly,z * z);
end;
...
begin
   if z < 0 then begin
      if (not invert) then
         Result := -ErfImp_1(-z, invert)
      else if (z < -0.5) then
         Result := 2 - ErfImp_1(-z, invert)
      else
         Result := 1 + ErfImp_1(-z, false);
      Exit;
   end;
   //
