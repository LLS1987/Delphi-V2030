...
begin
  ...
  Result.FLocalFile.LocalFileHeaderSignature := $04034B50;
  Result.FLocalFile.VersionNeededToExtract   := 20;
  Result.FLocalFile.GeneralPurposeBitFlag    := 0;
  Result.FLocalFile.CompressionMethod        := CM;
  Result.FLocalFile.LastModFileTimeDate      := DateTimeToFileDate(Now);
  Result.FLocalFile.Crc32                    := 0;
  Result.FLocalFile.CompressedSize           := 0;
  Result.FLocalFile.UncompressedSize         := UncompSz;
  Result.FLocalFile.FilenameLength           := Length(ItemName);
  Result.FLocalFile.ExtraFieldLength         := 0;
  Result.FLocalFile.FileName                 := AnsiString(ItemName);
  Result.FLocalFile.ExtraField               := '';
  Result.FLocalFile.CompressedDataPos        := 0;

  Result.FCentralDirectoryFile.CentralFileHeaderSignature     := $02014B50;
  ...
  Result.FCentralDirectoryFile.CompressionMethod              := CM;
  Result.FCentralDirectoryFile.LastModFileTimeDate            := DateTimeToFileDate(Now);
  Result.FCentralDirectoryFile.Crc32                          := 0;
  Result.FCentralDirectoryFile.CompressedSize                 := 0;
  Result.FCentralDirectoryFile.UncompressedSize               := UncompSz;
  ...
  Result.FCentralDirectoryFile.ExternalFileAttributes         := $00000020; // faArchive;
  Result.FCentralDirectoryFile.RelativeOffsetOfLocalHeader    := FEndOfCentralDir.OffsetOfStartOfCentralDirectory;
  Result.FCentralDirectoryFile.FileName                       := AnsiString(ItemName);
...
begin
  ...
  FCurrZip.FLocalFile.LocalFileHeaderSignature := $04034B50;
  FCurrZip.FLocalFile.VersionNeededToExtract   := 20;
  FCurrZip.FLocalFile.GeneralPurposeBitFlag    := 0;
  FCurrZip.FLocalFile.CompressionMethod        := CM;
  FCurrZip.FLocalFile.LastModFileTimeDate      := DateTimeToFileDate(Now);
  FCurrZip.FLocalFile.Crc32                    := 0;
  FCurrZip.FLocalFile.CompressedSize           := 0;
  FCurrZip.FLocalFile.UncompressedSize         := 0;
  FCurrZip.FLocalFile.FilenameLength           := Length(ItemName);
  FCurrZip.FLocalFile.ExtraFieldLength         := 0;
  FCurrZip.FLocalFile.FileName                 := AnsiString(ItemName);
  FCurrZip.FLocalFile.ExtraField               := '';
  FCurrZip.FLocalFile.CompressedDataPos        := 0;

  FCurrZip.FCentralDirectoryFile.CentralFileHeaderSignature     := $02014B50;
  FCurrZip.FCentralDirectoryFile.VersionMadeBy                  := 20;
  FCurrZip.FCentralDirectoryFile.VersionNeededToExtract         := 20;
  FCurrZip.FCentralDirectoryFile.GeneralPurposeBitFlag          := 0;
  FCurrZip.FCentralDirectoryFile.CompressionMethod              := CM;
  FCurrZip.FCentralDirectoryFile.LastModFileTimeDate            := DateTimeToFileDate(Now);
  FCurrZip.FCentralDirectoryFile.Crc32                          := 0;
  FCurrZip.FCentralDirectoryFile.CompressedSize                 := 0;
  FCurrZip.FCentralDirectoryFile.UncompressedSize               := 0;
  FCurrZip.FCentralDirectoryFile.FilenameLength                 := Length(ItemName);
  FCurrZip.FCentralDirectoryFile.ExtraFieldLength               := 0;
  FCurrZip.FCentralDirectoryFile.FileCommentLength              := 0;
  FCurrZip.FCentralDirectoryFile.DiskNumberStart                := 0;
  FCurrZip.FCentralDirectoryFile.InternalFileAttributes         := 0;
  FCurrZip.FCentralDirectoryFile.ExternalFileAttributes         := $00000020; // faArchive;
  FCurrZip.FCentralDirectoryFile.RelativeOffsetOfLocalHeader    := FEndOfCentralDir.OffsetOfStartOfCentralDirectory;
  FCurrZip.FCentralDirectoryFile.FileName                       := AnsiString(ItemName);
  FCurrZip.FCentralDirectoryFile.ExtraField                     := '';
  FCurrZip.FCentralDirectoryFile.FileComment                    := '';
...
function TXLSZipFile.Read(var Buffer; Count: LongInt): Longint;
var
  MaxSz: integer;
begin
  if FDStream <> Nil then begin
...
function TXLSZipFile.Write(const Buffer; Count: LongInt): Longint;
begin
