...
uses Classes, SysUtils, Types, Contnrs, Math,
{$ifdef MSWINDOWS}
     Windows,
  {$ifndef BABOON}
     Graphics, ComCtrls,
...
{$endif}
     ...
     XLSAutofilter5, XLSDrawing5, XLSDecodeFmla5,
...
{$ifdef XLS_RTF}
     XLSRTFUtils, XLSReadRTF, XLSWriteRTF, XLSRichPainter5, XLSRTFReadWrite5,
...
{$endif}
     ...
     function  GetTabSelected: boolean;
     procedure SetTabSelected(const Value: boolean);
...
public
     ...
     //* For internal use.
     function  RequestObject(AClass: string): TObject; override;
     //* For internal use.
...
{$ifndef AXAST}
     ...
     function  CalculateEx(Col, Row: integer; CalculateOptions: boolean): Variant; {$ifdef DELPHI_2007_OR_LATER} deprecated 'Use Calculate'; {$endif}
{$endif}
...
{$endif}
     ...
     property TabSelected: boolean read GetTabSelected write SetTabSelected;
...
begin
  ...
  for i := 0 to FOwner.Names.Count - 1 do begin
    ...
    if not Name.VbProcedure then begin
...
//          try RichPaint.AssignText(FManager.SST.ItemText[i],XF,Runs,Cnt);
...
//          S := FManager.SST.ItemText[i]; // setting width to current IterCellCol width if there is no merged column
//          textWidth := self.Columns[FCells.IterCellCol].CharWidth; // calc width merged cells width in chars
//          for i2 := 0 to self.MergedCells.count-1 do begin
//            if (self.MergedCells[i2].Ref.Col1 = FCells.IterCellCol) and (self.MergedCells[i2].Ref.Row1 = Fcells.IterCellRow) then
//              for i3 := self.MergedCells[i2].Ref.Col1 + 1 to self.MergedCells[i2].Ref.Col2 do begin
//                textWidth := textWidth + self.Columns[i3].CharWidth;
//              end;
//            end;
//            // wrap delimeted text
//            S := WrapText(S,Round(textWidth));
//            for i := 1 to Length(S) do begin
//              if S[i] = #13 then
//                Inc(n);
//              end;
//              // wrap not delimeted text
//              if n = 1 then begin
//                i := Length(S);
//                while i > Round(textWidth) do begin
//                  Inc(n); Dec(i,Round(textWidth));
//                end;
//              end;
//            end;
//            if H = 0 then begin
...
//            FManager.StyleSheet.Fonts.DefaultFont.CopyToTFont(Canvas.Font); FRows[ARow].Height := Fork(H,AMinHeight,AMaxHeight);
//            Result := True;
//          end;
//        end;
//      finally
//        ReleaseDC(GetDesktopWindow(),Canvas.Handle); Canvas.Free;
//      end;
//    end; {$endif}
...
begin
  ...
  try
    ...
    MaxH := 0;
    FCells.BeginIterate(ARow);
    while FCells.IterateNextCol do
    begin
    if FCells.IterCell.Data <> Nil then
	  begin
        XF := FManager.StyleSheet.XFs[FCells.IterGetStyleIndex];
        H := 0;
        n := 1;

        if (FCells.CellType(FCells.IterCell) = xctString) then
		    begin
          i := FCells.GetStringSST(FCells.IterCell);
          ...
          if SSTStringIsRichStr(FManager.SST[i]) then
          begin
            FCells.GetString(FCells.IterCell,Runs,Cnt);
          ...
          begin
            S := FManager.SST.ItemText[i];
             // setting width to current IterCellCol width if there is no merged column
            textWidth := self.Columns[FCells.IterCellCol].CharWidth;

            // calc merged cells width in chars
            for i2 := 0 to self.MergedCells.count-1 do
            begin
               if (self.MergedCells[i2].Ref.Col1 = FCells.IterCellCol) and
                  (self.MergedCells[i2].Ref.Row1 = Fcells.IterCellRow) then
                for i3 := self.MergedCells[i2].Ref.Col1+1 to self.MergedCells[i2].Ref.Col2 do
                begin
                  textWidth := textWidth + self.Columns[i3].CharWidth;
                end;
            end;
            ...
            begin
              i := Length(S);
              while i > Round(textWidth) do
              begin
                Inc(n);
                Dec(i,Round(textWidth));
              end;
            end;
          end;
        end;
    ...
    begin
      H := Round((MaxH / (Canvas.Font.PixelsPerInch / 72)) * 20);
      if (FCells.IterRow = Nil) or (H <> FCells.IterRow.Height) then
      begin
    ...
    while FCells.IterateNextRow do begin
      if FCells.FindCell(ACol,FCells.IterCellRow,C) and (FMergedCells.CellInAreas(ACol,FCells.IterCellRow) <= -1) then begin
        ...
        begin
          ...
          DrawTextExW(Canvas.Handle, PWideChar(S), -1, r, DT_CALCRECT, nil);
...
begin
//  Result := FTabColor;
  Result := FXc12Sheet.SheetPr.TabColor.ARGB;
end;

function TXLSWorksheet.GetTabSelected: boolean;
begin
  Result := FXc12SheetView.TabSelected;
end;
...
begin
  ...
  FHyperlinks.Move           (ADeltaCol,ADeltaRow);
...
function TXLSWorksheet.RequestObject(AClass: string): TObject;
begin
...
begin
  ...
  try
    ...
    Stream.Seek(Int64(0),soFromBeginning);
...
procedure TXLSWorksheet.SetTabSelected(const Value: boolean);
begin
  FXc12SheetView.TabSelected := True;
end;
...
//      Items[DestSheet].MergedCells.FillAndClipHitList(DestCol, DestRow,DestCol + (Col2 - Col1), DestRow + (Row2 - Row1),Items[SrcSheet].MergedCells);
      Items[SrcSheet].MergedCells.FillAndClipHitList(Col1, Row1, Col2, Row2,DestCol,DestRow,Items[DestSheet].MergedCells);
  end;
end;
...
begin
  ...
  EdgeHit := TSelectedEdgeHits(Byte((not Edges) and $0F));
end;
