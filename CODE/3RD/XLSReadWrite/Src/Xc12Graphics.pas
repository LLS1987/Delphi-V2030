...
public
     ...
     // For temporary use, For temporary use, determines that the data was loaded from memory stream.
     property WasLoadedFromStream: boolean read FWasLoadedFromStream;
     end;
...
begin
  ...
  while (jpeg.Position < n) and (b = $FF) do begin
    ...
    case b of
      $C0..$C3: begin
        jpeg.Seek(Int64(3),soFromCurrent);
        jpeg.Read(w,2);
        Height := Swap(w);
        jpeg.Read(w,2);
        Width := Swap(w);
...
//        BitDepth := b * 8;
      ...
      $FF: jpeg.Read(b,1);
      $D0..$D9, $01: begin
        jpeg.Seek(Int64(1),soFromCurrent);
        jpeg.Read(b,1);
      end;
      else begin
        jpeg.Read(w,2);
        jpeg.Seek(Int64(swap(w)-2), soFromCurrent);
        jpeg.Read(b,1);
      end;
    end;
  end;
...
begin
  ...
  Result.FRefCount := 1;
  Result.FWasLoadedFromStream := True;
...
begin
  ...
  AImage.Seek(Int64(0),soFromBeginning);
end;
