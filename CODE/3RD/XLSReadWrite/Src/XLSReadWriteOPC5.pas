...
const OPC_XLSX_VBAPROJECT       = 'http://schemas.microsoft.com/office/2006/relationships/vbaProject';
const OPC_XLSX_CUSTOMXML        = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/customXml';
const OPC_XLSX_CUSTOMXML_PROPS  = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/customXmlProps';
...
const OPC_XLSX_VBAPROJECT_TARGET       = 'vbaProject.bin';
...
begin
  ...
  try
    AStream.Seek(Int64(0),soFromBeginning);
  ...
  try
    if FindByType(ATypeName,List) then begin
      if List.Count > 1 then
        raise XLSRWException.Create('OPC: only one item of type ' + ATypeName + ' excpected');
      Result := List[0];
    end;
  finally
    List.Free;
...
begin
  ...
  if i >= 0 then begin
    ...
    if Result.Type_ <> AType then
      raise XLSRWException.Create('OPC Item is not of excpected type: ' + AType);
...
{$ifndef USE_AXZIP}
    raise XLSRWException.Create('No Zip. OSX.');
{$else}
...
begin
  ...
  for i := 0 to AList.Count - 1 do begin
    if (AList[i].TargetMode = otmInternal) and (AList[i].Type_ <> OPC_XLSX_WORKSHEET) then
...
begin
  ...
  if OPC = Nil then
    raise XLSRWException.Create('Can not find pivotCacheDefintion ' + AId + ' in OPC');
...
begin
  ...
  if OPCOwner <> Nil then begin
    ...
    if OPC = Nil then
      raise XLSRWException.Create('Can not find pivotCacheRecords '+ AId + ' in OPC');
    Result := ReadItemData(OPC);
    ItemClose(OPC);
  end
  else
    raise XLSRWException.Create('Can not find pivotCacheDefintion ' + AOwnerId + ' in OPC');
end;
...
begin
  ...
  if OPC = Nil then
    raise XLSRWException.Create('Can not find pivotTable ' + AId + ' in OPC');
  Result := ReadItemData(OPC);
...
function TOPC_XLSX.ReadVBA: TStream;
var
  Item: TOPCItem;
begin
  Item := ItemOpenRead(FWorkbook,OPC_XLSX_VBAPROJECT);
  if Item <> Nil then begin
    Result := ReadItemData(Item);
    ItemClose(Item);
  end
...
begin
  ...
  if OPC = Nil then
    raise XLSRWException.Create('Can not find VmlDrawing ' + AId + ' in OPC');
  Result := ReadItemData(OPC);
