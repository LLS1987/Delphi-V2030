...
{$ifdef AX_HAS_WINDOWS}
     Windows, ActiveX, ComObj, BIFF_VBA5,
{$endif}
{$ifdef AX_HAS_UITYPES}
     System.UITypes,
{$endif}
     xpgParseDrawing,
     Xc12Utils5, Xc12Manager5, Xc12DataWorkbook5, Xc12DefaultData5, Xc12Graphics,
{$ifdef XLS_BIFF}
...
{$endif}
     ...
     procedure DoVBA(AStream: TStream);
...
published
     ...
     property DoNotReadSheets: boolean read FDoNotReadSheets write FDoNotReadSheets;
...
var
  ...
  DrwPict  : TDrwPicture;
  PPIX,PPIY: integer;
  Img          : TXc12GraphicImage;
  ImgType      : TMSOBlipType;
  ImgData      : TStream;
  ImgFileName  : AxUCString;
  ...
  function TryBlipFromExt(Filename: AxUCString): TMSOBlipType;
  var
    Ext: AxUCString;
  begin
...
begin
  Img := AAnchor.Objects.Pic.BlipFill.Blip.Image;
  ImgFileName := AAnchor.Objects.Pic.NvPicPr.CNvPr.Descr;
  ImgType := TryBlipFromExt(ImgFileName);
  Result := (FBIFF <> Nil) and (ImgFileName <> '') and (ImgType <> msoblipUNKNOWN);
  if not Result then Exit;
  if Img.WasLoadedFromStream then
    begin
      ImgData := TPointerMemoryStream.Create();
      TPointerMemoryStream(ImgData).SetStreamData(Img.Image,Img.ImageSize);
      ImgData.Position := 0;
      Result := ImgData.Size > 16; // size of header image
    end
  else
  begin
    ImgData := Nil;
    Result := FileExists(ImgFileName);
  end;
  if Result then
  begin
    try
    ...
    MSO := FBIFF.MSOPictures.Add;
      ...
      if Assigned(ImgData) then
        MSO.LoadFromStream(ImgData,ImgFileName,ImgType)
      else
        MSO.Filename := ImgFileName;
...
{$ifdef SHAREWARE}
  for i := 0 to Count - 1 do
    Items[i].AsString[0,0] := 'XLSReadWriteII Copyright(c) 2022 Axolot Data';
{$endif}
...
{$ifdef XLS_BIFF}
procedure TXLSReadWriteII5.DoVBA(AStream: TStream);
var
  OleStorage: IStorage;
  DataHandle: HGLOBAL;
  Buffer    : PByteArray;
...
begin
  ...
  try
    ...
    OleCheck(StgOpenStorageOnILockBytes(LockBytes, Nil, STGM_READ or STGM_TRANSACTED or STGM_SHARE_DENY_NONE , Nil, 0, OleStorage));
  finally
  ...
  FVBA.ReadXLSM(OleStorage);

  OleCheck(LockBytes.Flush);
  LockBytes := Nil;
end;
{$else}
procedure TXLSReadWriteII5.DoVBA(AStream: TStream);
...
{$ifndef BABOON}
        XLSX.ExtractVBA := FReadVBA;
{$endif}

        XLSX.LoadFromStream(AStream,not FDoNotReadSheets);

        AfterRead;

        if XLSX.VBA <> Nil then
          DoVBA(XLSX.VBA);
      finally
        XLSX.Free;
      end;
