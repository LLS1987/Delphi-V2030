...
public
     ...
     function  LoadFromStream(AStream: TStream; ARestore: boolean = False): integer; override;
...
begin
  ...
  if ARequired then
    raise Exception.Create('XML error: Can not find expected node: ' + APathName);
  Result := Nil;
end;
...
function TXpgDOMNode.GetAsFloat: double;
begin
...
procedure TXpgDOMNode.SetAsFloat(const Value: double);
begin
...
function TXpgSimpleDOM.LoadFromStream(AStream: TStream; ARestore: boolean = False): integer;
begin
  Clear;
  FStack.Push(FRoot);
  try
