...
begin
  ...
  for i := 0 to FPostfixSz - 1 do begin
    ...
    case Ptgs.Id of
      ...
      xptg_OBJECT     : CopyPtgs(SizeOf(TXlsPtgsObject));
...
{$endif}
{$ifdef AXGDI}
      Obj := FManager.FindValue(TokenAsString(AToken));

      if Obj = Nil then begin
        Ptgs := AllocPtgs(SizeOf(TXLSPtgsErr),xptgErr);
        PXLSPtgsErr(Ptgs).Value := Byte(errName);

        FManager.Errors.Error('Unknown value ' + TokenAsString(AToken),0);
      end
      else begin
        Ptgs := AllocPtgs(SizeOf(TXLSPtgsObject),xptg_OBJECT);
        PXLSPtgsObject(Ptgs).Obj := Obj;
        AddPostfix(Ptgs);
      end;
{$endif}
    end;
