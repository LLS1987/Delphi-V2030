...
public
     ...
     function  FillAndClipHitList(const Col1,Row1,Col2,Row2: integer; AList: TBaseCellAreas; AObject: TObject = Nil): boolean; overload;
     function  FillAndClipHitList(const Col1,Row1,Col2,Row2,ADestCol, ADestRow: integer; AList: TBaseCellAreas): boolean; overload;
...
function TBaseCellAreas.FillAndClipHitList(const Col1, Row1, Col2, Row2, ADestCol, ADestRow: integer; AList: TBaseCellAreas): boolean;
var
  i: integer;
  Area: TCellArea;
  SelectArea,NewArea: TXLSCellArea;
begin
  ...
  for i := 0 to Count - 1 do begin
    if IntersectArea(Items[i].AsRecArea,SelectArea,NewArea) then begin
      ...
      Area.Col1 := Area.Col1 + (ADestCol - Col1);
      Area.Row1 := Area.Row1 + (ADestRow - Row1);
      Area.Col2 := Area.Col2 + (ADestCol - Col1);
      Area.Row2 := Area.Row2 + (ADestRow - Row1);
    end;
  end;
  Result := AList.Count > 0;
end;
...
begin
  ...
  for i := 0 to Count - 1 do begin
    ...
    if (C1 >= C.Col1) and (R1 >= C.Row1) and (C2 <= C.Col2) and (R2 <= C.Row2) then
      Exit;
    if (C.Col1 = C1) and (C.Col2 = C2) then begin
      if (R2 >= (C.Row1 - 1)) and (R2 <= C.Row2) then begin
        C.Row1 := R1;
        Exit;
      end
      else if (R1 >= C.Row1) and (R1 <= (C.Row2 + 1)) then begin
        C.Row2 := R2;
        Exit;
      end;
    end
    else if (C.Row1 = R1) and (C.Row2 = R2) then begin
      if (C2 >= (C.Col1 - 1)) and (C2 <= C.Col2) then begin
        C.Col1 := C1;
        Exit;
      end
      else if (C1 >= C.Col1) and (C1 <= (C.Col2 + 1)) then begin
        C.Col2 := C2;
        Exit;
      end;
    end;
