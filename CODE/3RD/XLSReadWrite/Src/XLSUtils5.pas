...
{$ifdef AX_HAS_UITYPES}
//type TColor = System.UITypes.TColor;
{$else}
...
const CurrentVersionNumber = '6.01.17';
// Max 4 digits
const CurrentBuildNumber = '6117';
...
type TAxGDIObjectNameProc = function(const AName: string): TObject of object;
...
function  TrimWS(const S: AxUCString): AxUCString;
...
protected
     FIndex: integer;
public
     function RequestObject(AClass: string): TObject; virtual;
...
begin
  for i := 1 to Length(AString) do begin
    if Word(AString[i]) > $00FF then begin
      Result := False;
      Exit;
    end;
  end;
...
function  RGBToTColor(const ARGB: longword): TColor;
begin
  Result := TColor(((ARGB and $000000FF) shl 16) + (ARGB and $0000FF00) + ((ARGB and $00FF0000) shr 16));
end;
...
begin
  L := Length(S);
  I := 1;
  if (L > 0) and (S[I] > ' ') and (S[L] > ' ') then begin
    Result := S;  
    Exit;
  end;  
  while (I <= L) and (CharInSet(S[I],[' ',#10,#13])) do Inc(I);
  if I > L then begin
    Result := '';  
    Exit;
  end;  
  while CharInSet(S[L],[' ',#10,#13]) do Dec(L);
...
begin
  ...
  EnumFontFamiliesEx(DC, LF, @EnumFontsProc, LPARAM(AList), 0);
  ReleaseDC(0, DC);
end;
{$endif}
...
function TIndexObject.RequestObject(AClass: string): TObject;
begin
  Result := Nil;
end;
