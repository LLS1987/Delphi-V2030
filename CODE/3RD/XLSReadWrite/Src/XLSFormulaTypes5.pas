...
type PXLSPtgsObject = ^TXLSPtgsObject;
     TXLSPtgsObject = packed record
     Id : byte;
     Obj: TObject;
     end;
...
begin
  ...
  if Data = Nil then
    raise XLSRWException.Create('Unknown function id: ' + IntToStr(AId));
  AMin := Data.Min;
  AMax := Data.Max;
...
{$endif}
  ...
  for i := 0 to High(Xc12Functions) do begin
    ...
    FList.Objects[Xc12Functions[i].Id] := TObject(@Xc12Functions[i]);
  end;

{$ifndef ANDROID}
  for i := 0 to High(Xc12FunctionArgs) do begin
    O := FList.Objects[Xc12FunctionArgs[i].FuncId];

    PXc12FuncData(O).Args := @Xc12FunctionArgs[i];
  end;
{$endif}
end;
