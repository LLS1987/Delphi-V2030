...
{$else}
     Xc12Manager5,
{$endif}
     Xc12DataXLinks5, Xc12DataWorkbook5, Xc12DataWorksheet5,
...
type TAxGDIFormulaValueEvent = procedure(AValue: TObject; out AResult: double) of object;
...
public
     ...
     procedure Push(const AValue: integer); overload; {$ifdef DELPHI_2006_OR_LATER} inline; {$endif}
...
protected
     ...
     FEvGDIValue   : TAxGDIFormulaValueEvent;
...
public
     ...
     property OnGDIValue: TAxGDIFormulaValueEvent read FEvGDIValue write FEvGDIValue;
     end;
...
begin
  ...
  while AArgCount > 0 do begin
    ...
    while FIterator.NextFloat do begin
      Sum := Sum + FIterator._Result.vFloat;
      Inc(N);
    end;
    if FIterator._Result.ValType = xfvtError then begin
      Push(FIterator._Result.vErr);
      Exit;
    end;
    Dec(AArgCount);
  end;
...
begin
  ...
  while FIterator.Next do begin
    FVToVarValue(@FIterator._Result,@Val);
    if CompareVarValue(@Val,@CritVal,Op) then begin
      ...
      if (AArgCount = 3) and FIterator.Linked.ResIsFloat then
        Sum2 := Sum2 + FIterator.Linked._Result.vFloat;
      Inc(n);
    end;
  end;
  if n <= 0 then
  ...
  for i := 0 to AArgCount - 1 do begin
    ...
    while FIterator.Next do begin
      if Err = errUnknown then begin
        Err := GetAsBoolean(FIterator._Result,B);
...
begin
  ...
  if not AreasEqualSize(@FV2,@FV1) or (AreaCellCount(@FV1) <= 1) then
    ...
    while FIterator.NextFloat do begin
      V2 := FIterator._Result.vFloat;
      V1 := FIterator.Linked._Result.vFloat;
      if V2 = 0 then begin
        Push(errDiv0);
        Exit;
      end;
...
begin
  ...
  FIterator.BeginIterate(PopRes,False);
  while FIterator.Next do begin
    if not FIterator._Result.Empty then begin
      case FIterator._Result.ValType of
        xfvtBoolean: begin
          if FIterator._Result.RefType = xfrtNone then
            if FIterator._Result.vBool then AddValue(1) else AddValue(0);
        end;
        xfvtFloat  : AddValue(FIterator._Result.vFloat);
        xfvtString : begin
          if (FIterator._Result.RefType = xfrtNone) and TryStrToFloat(FIterator._Result.vStr,V) then
            AddValue(V);
        end;
        xfvtError  : begin
          Push(FIterator._Result.vErr);
...
begin
  ...
  while FIterator.Next do begin
    if not FIterator._Result.Empty then begin
      case FIterator._Result.ValType of
        xfvtBoolean:  if FIterator._Result.vBool then AddValue(1) else AddValue(0);
        xfvtFloat  : AddValue(FIterator._Result.vFloat);
        xfvtString : begin
          if TryStrToFloat(FIterator._Result.vStr,V) then
            AddValue(V)
          else
            AddValue(0);
        end;
...
begin
  ...
  for i := AArgCount - 1 downto 0 do begin
    ...
    while FIterator.Next do begin
      Err := GetAsString(FIterator._Result,S);
      if Err = errUnknown then
        Res := Res + S;
    end;
  end;
...
begin
  ...
  if not AreasEqualSize(@FVX,@FVY) then
    ...
    while FIterator.NextFloat do begin
      X := FIterator._Result.vFloat;
      Y := FIterator.Linked._Result.vFloat;
...
begin
  ...
  while AArgCount > 0 do begin
    FIterator.BeginIterate(PopRes,False);
    while FIterator.Next do begin
      GetAsValueType(FIterator._Result,FVT);
...
begin
  ...
  FIterator.BeginIterate(PopRes,False);
  while FIterator.Next do begin
    if FIterator._Result.Empty or (FIterator._Result.ValType = xfvtUnknown) or ((FIterator._Result.ValType = xfvtString) and (FIterator._Result.vStr = '')) then
      Inc(Res);
  end;
  Push(Res);
end;
...
begin
  ...
  if not AreasEqualSize(@FVX,@FVY) then
    ...
    FIterator.BeginIterate(FVX,FVY,False);
    while FIterator.NextFloat do begin
      vX := FIterator._Result.vFloat;
      vY := FIterator.Linked._Result.vFloat;
...
begin
  ...
  if not AreasEqualSize(@FVX,@FVY) then
    ...
    FIterator.BeginIterate(FVX,FVY,False);
    while FIterator.NextFloat do begin
      V := FIterator._Result.vFloat;
      Sum1 := Sum1 + V;
      SumSq1 := SumSq1 + V * V;

      V := FIterator.Linked._Result.vFloat;
      Sum2 := Sum2 + V;
      SumSq2 := SumSq2 + V * V;
...
begin
  ...
  if Lookup.RefType = xfrtArray then
    Lookup := TXLSArrayItem(Lookup.vSource).GetAsFormulaValue(0,0);
...
begin
  ...
  while FIterator.Next do begin
    if FIterator._Result.ValType = fvValue.ValType then begin
      case fvValue.ValType of
        xfvtFloat   : begin
          if FIterator._Result.vFloat <= fvValue.vFloat then
            j := i;
        end;
        xfvtBoolean : begin
          if FIterator._Result.vBool = fvValue.vBool then
            j := i;
        end;
        xfvtString  : begin
          if CompareText(FIterator._Result.vStr,fvValue.vStr) <= 0 then
            j := i;
        end;
      end;
    end;
...
begin
  ...
  while FIterator.NextNotEmpty do begin
    if LookupVal.ValType = FIterator._Result.ValType then begin
      case MatchType of
       -1: begin
          iLast := ResultIndex;
          if IsFirst then begin
            if CompareFVValue(@LookupVal,@FIterator._Result,xdcoGT) then begin
              Push(errNA);
              Exit;
            end;
            IsFirst := False;
          end;
          if CompareFVValue(@FIterator._Result,@LookupVal,xdcoLT) then begin
            if not CompareFVValue(@FIterator._Result,@LookupVal,xdcoEQ) then
              Push(ResultIndex - 1)
            else
              Push(ResultIndex);
            Exit;
        ...
        0: begin
          if CompareFVValue(@FIterator._Result,@LookupVal,xdcoEQ) then begin
            Push(ResultIndex);
            Exit;
          end;
        end;
        1: begin
          iLast := ResultIndex;
          if IsFirst then begin
            if CompareFVValue(@LookupVal,@FIterator._Result,xdcoLT) then begin
              Push(errNA);
              Exit;
            end;
            IsFirst := False;
          end;
          if CompareFVValue(@FIterator._Result,@LookupVal,xdcoGT) then begin
            if not CompareFVValue(@FIterator._Result,@LookupVal,xdcoEQ) then
              Push(ResultIndex - 1)
            else
              Push(ResultIndex);
...
begin
  ...
  while AArgCount > 0 do begin
    ...
    while FIterator.NextFloat do begin
      Inc(n);
      if FIterator._Result.vFloat > vMax then
        vMax := FIterator._Result.vFloat;
    end;
    if FIterator._Result.ValType = xfvtError then begin
      Push(FIterator._Result.vErr);
      Exit;
...
begin
  ...
  while AArgCount > 0 do begin
    ...
    while FIterator.NextFloat do begin
      Inc(n);
      if FIterator._Result.vFloat < vMin then
        vMin := FIterator._Result.vFloat;
    end;
    if FIterator._Result.ValType = xfvtError then begin
      Push(FIterator._Result.vErr);
      Exit;
...
begin
  ...
  while FIterator.NextFloat do begin
    vVal := FIterator._Result.vFloat;
    vProb := FIterator.Linked._Result.vFloat;
...
begin
  ...
  while AArgCount > 0 do begin
    FIterator.BeginIterate(PopRes,False);
    while FIterator.Next do begin
      if GetAsFloatNum(FIterator._Result,V,Err) then begin
        if Err <> errUnknown then begin
          Push(Err);
          Exit;
        end;
...
begin
  ...
  FIterator.BeginIterate(FV,False);
  while FIterator.Next do begin
    GetAsFloat(FIterator._Result,V,Err);
    if Err <> errUnknown then begin
      Push(Err);
      Exit;
    end;
...
begin
  ...
  if not AreasEqualSize(@FVX,@FVY) then
    ...
    FIterator.BeginIterate(FVX,FVY,False);
    while FIterator.NextFloat do
      Res := Res + Sqr(FIterator._Result.vFloat) - Sqr(FIterator.Linked._Result.vFloat);
    Push(Res);
  end;
end;
...
begin
  ...
  if not AreasEqualSize(@FVX,@FVY) then
    ...
    FIterator.BeginIterate(FVX,FVY,False);
    while FIterator.NextFloat do
      Res := Res + Sqr(FIterator._Result.vFloat - FIterator.Linked._Result.vFloat);
  end;
end;
...
begin
  ...
  for i := 0 to AArgCount - 1 do begin
    ...
    while FIterator.Next do begin
      case FIterator._Result.ValType of
        xfvtFloat   : Variance := Variance + Sqr(FIterator._Result.vFloat - Average);
        xfvtError   : begin
          Push(FIterator._Result.vErr);
          Exit;
        end;
...
begin
  ...
  while FIterator.NextFloat do begin
    ASum1 := ASum1 + FIterator._Result.vFloat;
    ASum2 := ASum2 + FIterator.Linked._Result.vFloat;
    Inc(Result);
  end;
end;
...
begin
  ...
  // Functions with required first parameter.
  case AId of
    344: Dec(AArgCount); // SUBTOTAL
  end;
...
{$endif}
      ...
      xptg_OBJECT: begin
        if Assigned(FEvGDIValue) then begin
          FEvGDIValue(PXLSPtgsObject(APtgs).Obj,V);

          FStack.Push(V);
        end;

        APtgs := PXLSPtgs(NativeInt(APtgs) + SizeOf(TXLSPtgsObject));
      end
...
procedure TValueStack.Push(const AValue: integer);
begin
  IncStackPtr;
  FStack[FStackPtr].Empty := False;
  FStack[FStackPtr].RefType := xfrtNone;
...
begin
  Result := 0;
  case _Result.ValType of
    xfvtFloat   : Result := _Result.vFloat;
    xfvtBoolean : if _Result.vBool then Result := 1 else Result := 0;
  end;
end;
...
begin
  if not FIgnoreError and not FHaltOnError then
    _Result.ValType := xfvtUnknown;
  _Result.RefType := xfrtNone;
  Result := False;
  if FDone then
    Exit;
  ...
  case FFV.RefType of
    xfrtNone,
    xfrtRef      : begin
      _Result.Empty := FFV.Empty;
      if not _Result.Empty then begin
        _Result.ValType := FFV.ValType;
        case FFV.ValType of
          xfvtFloat   : _Result.vFloat := FFV.vFloat;
          xfvtBoolean : _Result.vBool := FFV.vBool;
          xfvtError   : _Result.vErr := FFV.vErr;
          xfvtString  : _Result.vStr := FFV.vStr;
        end;
      end;
      FDone := True;
    ...
    xfrtArea     : begin
      ...
      if Result and FIgnoreHidden then
        Result := DoOptions;
      if Result then
        FOwner.GetCellValue(TXLSCellMMU(FSource),FCol,FRow,@_Result);
    end;
    xfrtXArea    : ;
    xfrtAreaList : begin
      ...
      if Result and FIgnoreHidden then
        Result := DoOptions;
      if Result then
        FOwner.GetCellValue(TXLSCellMMU(CA.Obj),FCol,FRow,@_Result);
    end;
    xfrtArray    : begin
      Inc(FCol);
      ...
      Result := not FDone;
      if Result then
        _Result := TXLSArrayItem(FSource).GetAsFormulaValue(FCol,FRow);
    end;
  end;
  if not FIgnoreError and FHaltOnError and (_Result.ValType = xfvtError) then
    FDone := True;
  if FIgnoreError and not FDone and (_Result.ValType = xfvtError) then
    Result := DoNext;
end;
...
begin
  Result := _Result.ValType = xfvtError;
  if Result then
    AError := _Result.vErr
...
begin
  Result := _Result.ValType = xfvtFloat;
...
begin
  Result := _Result.ValType in [xfvtFloat,xfvtBoolean,xfvtString];
  if Result and (FLinked <> Nil) then
...
begin
  Result := _Result.ValType <> xfvtUnknown;
  if Result and (FLinked <> Nil) then
    Result := FLinked.ResIsFloat;
end;
