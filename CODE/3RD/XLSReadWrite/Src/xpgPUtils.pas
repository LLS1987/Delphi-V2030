...
uses Classes, SysUtils, XLSUtils5;
...
// function  SplitAtChar(const C: AxUCChar; var S: string): string; overload;
// function  SplitAtChar(const C: AxUCChar; var S: AxUCString): AxUCString; overload;
...
function XmlTryStrToFloat(const AValue: AxUCString; out AResult: double): boolean; overload;
//function XmlTryStrToFloat(const AValue: AxUCString; out AResult: extended): boolean; overload;
...
const
  xpgCRC32_POLYNOMIAL = $EDB88320;

  ReTablebase64: AnsiString =
...
var
  xpgCcitt32Table: array[0..255] of longword;

procedure xpgBuildCRCTable;
var i, j: longint;
    value: longword;
begin
  for i := 0 to 255 do begin
    ...
    for j := 8 downto 1 do
      if ((value and 1) <> 0) then
        value := (value shr 1) xor xpgCRC32_POLYNOMIAL
      else
        value := value shr 1;
    xpgCcitt32Table[i] := value;
  end
end;
...
begin
  ...
  l := NativeInt(p) - NativeInt(pointer(result));
  SetLength(Result, l);
end;
...
//function SplitAtChar(const C: AxUCChar; var S: string): string;
//var
//  p: integer;
//begin
//  p := CPos(C,S);
...
function XmlTryStrToFloat(const AValue: AxUCString; out AResult: double): boolean;
var
  TempDS: AxUCChar;
begin
  TempDS := FormatSettings.DecimalSeparator;
...
begin
  ...
  for i := 1 to Length(Str) do begin
    if (Word(Str[i]) and $FF00) <> 0 then begin
      ...
      Result:= (((Result shr 8) and $00FFFFFF) xor (xpgCcitt32Table[(Result xor b) and $FF]));
    end;
