...
procedure TXLSStringHash.Add(const Key: AxUCString; Value: Integer);
var
  Hash: Integer;
  Bucket: PHashItem;
begin
...
procedure TXLSStringHash.Clear;
var
  i: Integer;
  P, N: PHashItem;
begin
...
constructor TXLSStringHash.Create(Size: Cardinal);
begin
  inherited Create;
  SetLength(Buckets, Size);
end;

destructor TXLSStringHash.Destroy;
begin
  Clear;
...
function TXLSStringHash.Find(const Key: AxUCString): PPHashItem;
var
  Hash: Integer;
begin
  Hash := HashOf(Key) mod Cardinal(Length(Buckets));
...
function TXLSStringHash.HashOf(const Key: AxUCString): Cardinal;
var
  i: Integer;
begin
  Result := 0;
...
function TXLSStringHash.Modify(const Key: AxUCString; Value: Integer): Boolean;
var
  P: PHashItem;
begin
  P := Find(Key)^;
...
procedure TXLSStringHash.Remove(const Key: AxUCString);
var
  P: PHashItem;
  Prev: PPHashItem;
begin
...
function TXLSStringHash.ValueOf(const Key: AxUCString): Integer;
var
  P: PHashItem;
begin
  P := Find(Key)^;
...
begin
  ...
  FHashTable := TXLSStringHash.Create($FFFF);
...
//        Move(PByteArray(@Data)^,Pointer(S8)^,XString.Len);
      ...
      Move(PByteArray(@PXLSStringRich(XString)^.Data)^,Pointer(S8)^,XString.Len);
...
//        Move(PByteArray(@Data)^,Pointer(Result)^,XString.Len * 2);
      ...
      Move(PByteArray(@PXLSStringRichUC(XString)^.Data)^,Pointer(Result)^,XString.Len * 2);
    end;
    STRID_FAREAST: begin
      SetLength(S8,XString.Len);
...
//        Move(PByteArray(@Data)^,Pointer(S8)^,XString.Len);
      ...
      Move(PByteArray(@PXLSStringFarEast(XString)^.Data)^,Pointer(S8)^,XString.Len);
...
//        Move(PByteArray(@Data)^,Pointer(S8)^,XString.Len);
      ...
      Move(PByteArray(@PXLSStringFarEastRich(XString)^.Data)^,Pointer(S8)^,XString.Len);
...
//        Move(PByteArray(@Data)^,Pointer(Result)^,XString.Len * 2);
      ...
      Move(PByteArray(@PXLSStringFarEastUC(XString)^.Data)^,Pointer(Result)^,XString.Len * 2);
    end;
...
//        Move(PByteArray(@Data)^,Pointer(Result)^,XString.Len * 2);
      ...
      Move(PByteArray(@PXLSStringFarEastRichUC(XString)^.Data)^,Pointer(Result)^,XString.Len * 2);
    end;
  end;
end;
