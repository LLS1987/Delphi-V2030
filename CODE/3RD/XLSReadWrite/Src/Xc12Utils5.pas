...
uses Classes, SysUtils, Contnrs, Math, {$ifndef DELPHI_7}System.UITypes,{$endif}
//     System.UITypes,
     XLSUtils5;
...
function  Xc12ColorToRGB(Color: TXc12Color; AAutoColor: TXc12RGBColor = $00000000): TXc12RGBColor; overload;// {$ifdef D2006PLUS} inline; {$endif}
function  Xc12ColorToRGB(Color: PXc12Color; AAutoColor: TXc12RGBColor = $00000000): TXc12RGBColor; overload;// {$ifdef D2006PLUS} inline; {$endif}
...
function Xc12ColorToRGB(Color: PXc12Color; AAutoColor: TXc12RGBColor = $00000000): longword;
var
  RGB: TRGBRec;
  HLS: THLSRec;
//  RRec: TRGBRec;
...
begin
  case Color.ColorType of
    ...
    exctIndexed: begin
      if Integer(Color.Indexed) > High(Xc12IndexColorPaletteRGB) then
        Result := AAutoColor
      else
        Result := Xc12IndexColorPaletteRGB[Integer(Color.Indexed)];
    end;
    exctRgb    : begin
...
function Xc12ColorToRGB(Color: TXc12Color; AAutoColor: TXc12RGBColor = $00000000): longword;
begin
...
begin
  ...
  if (Color < 0) or (Color > Integer(High(TXc12IndexColor))) then begin
    ...
    Result.ARGB := Xc12IndexColorPaletteRGB[Color];
  end;
end;
...
function Xc12ColorEqual(const ColorXc12: TXc12Color; const ColorRGB: TXc12RGBColor): boolean;
begin
  Result := (ColorXc12.ColorType = exctRgb) and (ColorXc12.OrigRGB = ColorRGB);
end;
...
function RGBColorToXc12(const ARGB: TXc12RGBColor): TXc12Color;
begin
...
function  RGBColorToXc12(const ARGB: TXc12RGBColor; Color: PXc12Color): TXc12Color;
begin
...
begin
  ...
  Result.Tint := Fork(ATint,-1.0,1.0);
end;
...
begin
  ...
  AColor.Tint := Fork(ATint,-1.0,1.0);
end;
