...
procedure zlibFreeMem(opaque: pointer; items: uInt; size: uInt); cdecl;
begin
  FreeMem(opaque);
end;
...
{$ifdef COMPILE_LAZARUS}
        strm.next_out := pBytef(NativeInt(OutBuf) + (NativeInt(strm.next_out) - NativeInt(P)));
{$else}
...
function TDecompressionStream.ReadZip(var Buffer; Count: LongInt): Longint;
begin
...
function TDecompressionStream.Read_(var Buffer; Count: LongInt): Longint;
begin
  FZRec.next_out := @Buffer;
  FZRec.avail_out := Count;
  if FStrm.Position <> FStrmPos then FStrm.Position := FStrmPos;
