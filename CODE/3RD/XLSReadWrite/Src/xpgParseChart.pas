...
{$ifndef AXAST}
     Variants, // Braune Added 22.08.2020
{$endif}
...
type TST_Orientation_2 =  (stoMaxMin,stoMinMax);
const StrTST_Orientation_2: array[0..1] of AxUCString = ('maxMin','minMax');
...
public
  ...
  private
    procedure SetF(const Value: AxUCString);
protected
     FF: AxUCString;
...
public
     ...
     property F: AxUCString read GetF write SetF;
...
public
     ...
     property Val: TST_Orientation_2 read FVal write FVal;
     end;
...
public
     ...
     function  AddSerie(ARef: TXLSRelCells; AMarker: TST_MarkerStyle): TCT_LineSer;
...
public
     ...
     procedure SetMinMax(AMin, AMax: double); overload;
     // Braune - Added 22.08.2020
{$ifndef AXAST}
     procedure SetMinMax(AMin, AMax: Variant); overload;
{$endif}
     procedure SetMinAuto;
     procedure SetMaxAuto;
...
procedure TCT_StrRef.SetF(const Value: AxUCString);
begin
  FF := Value;
end;
...
begin
  ...
  if (FStrCache <> Nil) and FStrCache.Assigned then 
    if xaElements in FStrCache.Assigneds then
    begin
...
begin
  if (FManualLayout <> Nil) and FManualLayout.Assigned then 
    if xaElements in FManualLayout.Assigneds then
    begin
...
begin
  if FVal <> stoMinMax then
    AWriter.AddAttribute('val',StrTST_Orientation_2[Integer(FVal)]);
end;

procedure TCT_Orientation.AssignAttributes(AAttributes: TXpgXMLAttributeList);
begin
  if AAttributes[0] = 'val' then
    FVal := TST_Orientation_2(StrToEnum('sto' + AAttributes.Values[0]))
  else
    FOwner.Errors.Error(xemUnknownAttribute,AAttributes[0]);
end;
...
procedure TCT_AreaSer.SetColorRGB(const Value: longword);
begin
  FEG_SerShared.SetFillColor(Value);
end;
...
function TCT_AreaSer.GetColorRGB: longword;
begin
  Result := FEG_SerShared.GetFillColor;
end;
...
procedure TCT_LineSer.SetColorRGB(const Value: longword);
begin
  FEG_SerShared.SetLineColor(Value);
end;
...
function TCT_LineSer.GetColorRGB: longword;
begin
  Result := FEG_SerShared.GetLineColor;
end;
...
procedure TCT_BarSer.SetColorRGB(const Value: longword);
begin
  FEG_SerShared.SetFillColor(Value);
end;
...
function TCT_BarSer.GetColorRGB: longword;
begin
  Result := FEG_SerShared.GetFillColor;
end;
...
function TEG_LineChartShared.AddSerie(ARef: TXLSRelCells; AMarker: TST_MarkerStyle): TCT_LineSer;
begin
  Result := FSeries.Add;
  Result.Shared.Create_Idx.Val := FSeries.Count - 1;
  Result.Create_Val;
  Result.Val.Create_NumRef;
  Result.Val.NumRef.RCells := ARef;
  Result.Marker.Symbol.Val := AMarker;
end;
...
begin
  ...
  if FMajorGridlines <> Nil then
    Inc(ElemsAssigned,FMajorGridlines.CheckAssigned);
  if FMinorGridlines <> Nil then
    Inc(ElemsAssigned,FMinorGridlines.CheckAssigned);
  if FTitle <> Nil then
...
procedure TCT_ValAx.SetMinMax(AMin, AMax: double);
begin
  FEG_AxShared.Scaling.Create_Min.Val := AMin;
  FEG_AxShared.Scaling.Create_Max.Val := AMax;
end;

procedure TCT_ValAx.SetMaxAuto;
begin
  if (FEG_AxShared.Scaling.Max <> Nil) then begin
    FEG_AxShared.Scaling.Free;
    FEG_AxShared.Scaling.FMax := Nil;
  end;
end;

procedure TCT_ValAx.SetMinAuto;
begin
  if (FEG_AxShared.Scaling.Min <> Nil) then begin
    FEG_AxShared.Scaling.Min.Clear;
    FEG_AxShared.Scaling.FMin := Nil;
  end;
end;

// Braune - Added 22.08.2020
{$ifndef AXAST}
procedure TCT_ValAx.SetMinMax(AMin, AMax: Variant);
begin
  if VarIsNull(aMin) then begin
    if (FEG_AxShared.Scaling.FMin <> Nil) then begin
      FEG_AxShared.Scaling.FMin.Clear;
      FEG_AxShared.Scaling.FMin := Nil;
    end;
  end else
    FEG_AxShared.Scaling.Create_Min.Val := AMin;

  if VarIsNull(aMax) then begin
    if (FEG_AxShared.Scaling.FMax <> Nil) then begin
      FEG_AxShared.Scaling.FMax.Clear;
      FEG_AxShared.Scaling.FMax := Nil;
    end;
  end else
    FEG_AxShared.Scaling.Create_Max.Val := AMax;
...
begin
  ...
  if (FChartSpace <> Nil) and FChartSpace.Assigned then begin
    if xaElements in FChartSpace.Assigneds then
    begin
...
constructor TXPGDocXLSXChart.Create(AGrManager: TXc12GraphicManager);
begin
...
var
  R: TXPGReader;
begin
  R := TXPGReader.Create(FErrors,FRoot);
  try
