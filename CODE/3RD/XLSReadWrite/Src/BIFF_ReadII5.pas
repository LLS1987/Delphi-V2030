...
begin
  ...
  with PRecNUMBER(PBuf)^ do begin
    if FXLS.Version < xvExcel40 then
      ...
      FormatIndex := FormatIndex + XLS_STYLE_DEFAULT_XF_COUNT;

    if FManager.DirectRead then
      DoDirectRead(Col,Row,Value)
    else
      FCurrXc12Sheet.Cells.StoreFloat(Col,Row,FormatIndex,Value);
  end;
end;
  ...
  with PRecLABEL(PBuf)^ do begin
    if FXLS.Version >= xvExcel97 then begin
      ...
      if FManager.DirectRead then
        ...
        FCurrXc12Sheet.Cells.StoreString(Col,Row,FormatIndex,WS);
    end;
  end;
end;
...
//    XF.E97XfData7 := X8.Data7;
    ...
    if X8.FontIndex < FManager.StyleSheet.Fonts.Count then
      XF.Font := FManager.StyleSheet.Fonts[X8.FontIndex + XLS_STYLE_DEFAULT_FONT_COUNT]
    else
      XF.Font := FManager.StyleSheet.Fonts[XLS_STYLE_DEFAULT_FONT];

    if X8.NumFmtIndex < FManager.StyleSheet.NumFmts.Count then
...
begin
  ...
  for i := 0 to Count - 1 do begin
    ...
    if pFmt.FontIndex < FManager.StyleSheet.Fonts.Count then
      Result[i].Font := FManager.StyleSheet.Fonts[pFmt.FontIndex + XLS_STYLE_DEFAULT_FONT_COUNT]
    else
      Result[i].Font := FManager.StyleSheet.Fonts[XLS_STYLE_DEFAULT_FONT];
  end;
end;
