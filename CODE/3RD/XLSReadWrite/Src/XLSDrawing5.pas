...
public
     ...
     function MakeLineChart(ASrcArea: TXLSRelCells; ACol,ARow: integer; A3D: boolean = False; AMarker: TST_MarkerStyle = stmsNone): TCT_ChartSpace;
...
var
  i: integer;
begin
  for i := FDrw.Anchors.TwoCellAnchors.Count - 1 downto 0 do begin
    if (FDrw.Anchors.TwoCellAnchors[i].From.Col >= longword(ACol1)) and (FDrw.Anchors.TwoCellAnchors[i].From.Row >= longword(ARow1)) and
       (FDrw.Anchors.TwoCellAnchors[i].To_.Col <= longword(ACol2)) and (FDrw.Anchors.TwoCellAnchors[i].To_.Row <= longword(ARow2)) then begin
      FDrw.Anchors.TwoCellAnchors.Delete(i);
    end;
  end;
end;
...
procedure TXLSDrawingObjects.Delete(AIndex: integer);
begin
...
{$ifdef BABOON}
      Result := $FFFFFF;
{$else}
      Result := Graphics.ColorToRGB(clWindow);
{$endif}
    end;
    xdxctScheme : begin
...
function TXLSDrawingCharts.MakeLineChart(ASrcArea: TXLSRelCells; ACol, ARow: integer; A3D: boolean = False; AMarker: TST_MarkerStyle = stmsNone): TCT_ChartSpace;
var
  i     : integer;
  Chart : TXPGDocXLSXChart;
  Line  : TCT_LineChart;
...
begin
  ...
  for i := ASrcArea.Col1 to ASrcArea.Col2 do
    Shared.AddSerie(ASrcArea.CloneCol(i), AMarker);
...
destructor TXLSDrwText.Destroy;
begin
  if FBody <> Nil then
    FBody.Free;
