...
function TXLSOwnerStream.Read(var Buffer; Count: Longint): Longint;
begin
...
function TXLSOwnerStream.Write(const Buffer; Count: Longint): Longint;
begin
...
begin
  ...
  if FWriteStream <> Nil then begin
    ...
    Magic[2] := $03;
    Magic[3] := $04;
    FWriteStream.Seek(Int64(0),soFromBeginning);
...
begin
  i := Find(Filename);
  if i < 0 then
    raise XLSRWException.Create('Can not find file ' + Filename + ' in ZIP');
...
begin
  Source.Seek(Int64(0),soFromBeginning);
