...
begin
  ...
  // Excel will crash on print if there not is at least one BookView.
  if FManager.Workbook.BookViews.Count < 1 then
    FManager.Workbook.BookViews.Clear(True);
...
begin
  ...
  if AWriteElement then begin
    ...
    if (R.SpanC1 <> $FFFF) and (R.SpanC2 <> $FFFF) then
      Row.SpansXpgList.Add(IntToStr(R.SpanC1) + ':' + IntToStr(R.SpanC2));
