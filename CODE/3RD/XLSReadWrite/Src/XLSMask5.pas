...
var
  ...
  C: char;
  pMD,
  LastTimeMD: TMaskData;
  S: string;
  FD: PFormatData;
begin
  FD := @Formats[Index];

  FD.Data := TMaskDataList.Create;
  FD.Flags := [];
  if Index = 2 then
    FD.Flags := FD.Flags + [mfZero];
  FD.Color := $000000;
  FD.DecimalCount := 0;
  FD.DataDecimalPos := -1;
  FD.Div1000Cnt := 0;
  ...
  while i <= Length(Value) do begin
    case Value[i] of
      ...
      begin
        ...
        if mfDecimals in FD.Flags then
          Inc(FD.DecimalCount);
        FD.Div1000Cnt := 0;
      end;
      '?':
      ...
      'e','E': begin
        if (i < Length(Mask)) and CharInSet(Mask[i + 1],['-','+']) then begin
          Inc(i);
          AddEntity(Index,meNone);
          FD.Flags := FD.Flags + [mfScientific];
        end
        else begin
...
//            pMD := AddEntity(Index,meRightIndent);
      ...
      '.':
      begin
        FD.Flags := FD.Flags + [mfDecimals];
        AddEntity(Index,meDecimalPos);
        FD.DataDecimalPos := FD.Data.Count;
      end;
      ',': begin
        FD.Flags := FD.Flags + [mfThousand];
        Inc(FD.Div1000Cnt);
      end;
      '[':
      begin
        ...
        if i <= Length(Value) then begin
          ...
          else if S = 'BLACK'   then FD.Color := $000000
          else if S = 'CYAN'    then FD.Color := $FFFF00
          else if S = 'MAGENTA' then FD.Color := $FF00FF
          else if S = 'WHITE'   then FD.Color := $FFFFFF
          else if S = 'BLUE'    then FD.Color := $FF0000
          else if S = 'GREEN'   then FD.Color := $00FF00
          else if S = 'RED'     then FD.Color := $0000FF
          else if S = 'YELLOW'  then FD.Color := $00FFFF;
        end;
        if (FD.Data.Count > 0) and (FD.Data[FD.Data.Count - 1].Entity in [meHourElapsed,meMinuteElapsed,meSecondElapsed]) then
          FD.Flags := FD.Flags + [mfIsDateTime];
      end;
      'Y','M','D','y','m','d','H','S','h','s':
      begin
        FD.Flags := FD.Flags + [mfIsDateTime];
        ...
        LastTimeMD := FD.Data[FD.Data.Count - 1];
        Dec(i);
      end;
      else begin
        pMD := AddEntity(Index,meLitteral);
        ...
        if CharInSet(Value[i],['a','A']) then begin
          ...
          if pMD.Entity in [meStrap,meStrAmPmLow,meStrAmPmUpp] then begin
            for j := FD.Data.Count - 2 downto 0 do begin
              if FD.Data[j].Entity = meHourDig1 then begin
                FD.Data[j].Entity := meHourDig1AmPm;
                Break;
              end
              else if FD.Data[j].Entity = meHourDig2 then begin
                FD.Data[j].Entity := meHourDig2AmPm;
                Break;
              end
            end;
          end;
  ...
  for i := 0 to FD.Data.Count - 1 do begin
    if FD.Data[i].Entity in [mePlace,meSpace,meZero,meDecimalPos] then begin
      if not (FD.Data[i].Entity = meDecimalPos) then
        FD.Flags := FD.Flags - [mfIsDateTime];
      pMD := FD.Data.Insert(i);
      pMD.Entity := meFirstDigitPlace;
      Break;
    end;
  end;
  if FD.Div1000Cnt > 4 then
    FD.Div1000Cnt := 4;
end;
