...
{$endif}
     function  MakeCommentFont(const ABold: boolean): TXc12Font; overload;
     function  MakeCommentFont(const AName: AxUCString; ASize: integer; ABold, AItalic: boolean): TXc12Font; overload;
...
begin
  for i := 0 to Count - 1 do begin
    if Items[i].FgColor.ColorType = exctIndexed then
      Items[i].FFgColor.ARGB := Xc12IndexColorPaletteRGB[Integer(Items[i].FFgColor.Indexed)];
    if Items[i].FBgColor.ColorType = exctIndexed then
      Items[i].FBgColor.ARGB := Xc12IndexColorPaletteRGB[Integer(Items[i].FBgColor.Indexed)];
...
begin
  for i := 0 to Count - 1 do begin
    if Items[i].Color.ColorType = exctIndexed then
      Items[i].FColor.ARGB := Xc12IndexColorPaletteRGB[Integer(Items[i].Color.Indexed)];
  end;
end;
...
begin
  for i := 0 to Count - 1 do begin
    if Items[i].Color.ColorType = exctIndexed then
      Items[i].FColor.ARGB := Xc12IndexColorPaletteRGB[Integer(Items[i].FColor.Indexed)];
  end;
end;
...
function TXc12Fonts.MakeCommentFont(const AName: AxUCString; ASize: integer; ABold, AItalic: boolean): TXc12Font;
var
  Font: TXc12Font;
begin
  ...
  if AItalic then
    Font.Style := Font.Style + [xfsItalic];
...
begin
  for i := 0 to High(AFonts) do begin
    // Fonts can be Nil in non-excel created files.
...
var
  XF: TXc12XF;
begin
//  if AIndex >= FStyles.XFs.Count then
//    Exit;
