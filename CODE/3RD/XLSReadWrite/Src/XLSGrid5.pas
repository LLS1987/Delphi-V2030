...
uses Classes, SysUtils, Math, Controls, Grids, Tabs, Windows,
...
published
     ...
     property OnTabChanged : TNotifyEvent read FEvTabChanged write FEvTabChanged;
      ...
      for j := 0 to Para.TextRuns.Count - 1 do begin
        ...
        if Run.RPr.Available then begin
          ...
          if Run.RPr.FillProperties.SolidFill <> Nil then
            Font.PColor.ARGB := SolidFillToRGB(Run.RPr.FillProperties.SolidFill);
        end;
...
begin
  if FSheet.CellType[ACol - 1,ARow - 1] in XLSCellTypeFormulas then
...
begin
  ...
  if Assigned(FEvTabChanged) then
    FEvTabChanged(Self);
end;
