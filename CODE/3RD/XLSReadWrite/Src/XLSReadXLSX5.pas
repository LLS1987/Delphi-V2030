...
protected
     ...
     procedure GetBordersStyle(ASrcBorder: TCT_Border; ADestBorder: TXc12Border);
...
public
     ...
     procedure LoadFromStream(AZIPStream: TStream; AReadSheets: boolean = True);
     procedure LoadSheetNamesFromStream(AZIPStream: TStream; AList: TStrings);

     property ExtractVBA: boolean read FExtractVBA write FExtractVBA;
     property VBA: TStream read FVBA;
     end;
...
procedure TXLSReadXLSX.GetBordersStyle(ASrcBorder: TCT_Border; ADestBorder: TXc12Border);
begin
  ADestBorder.Options := [];
  if ASrcBorder.DiagonalUp then
    ADestBorder.Options := ADestBorder.Options + [ecboDiagonalUp];
  ...
  if ASrcBorder.Left.IsRead then
    GetBorder(ASrcBorder.Left,ADestBorder.Left);
  if ASrcBorder.Right.IsRead then
    GetBorder(ASrcBorder.Right,ADestBorder.Right);
  if ASrcBorder.Top.IsRead then
    GetBorder(ASrcBorder.Top,ADestBorder.Top);
  if ASrcBorder.Bottom.IsRead then
    GetBorder(ASrcBorder.Bottom,ADestBorder.Bottom);
  GetBorder(ASrcBorder.Diagonal,ADestBorder.Diagonal);
  GetBorder(ASrcBorder.Vertical,ADestBorder.Vertical);
  GetBorder(ASrcBorder.Horizontal,ADestBorder.Horizontal);
...
//    if AColor.RGB < 0 then
//      Result.OrigRGB := $000000
//    else
// 220808 Keep alpha value
...
procedure TXLSReadXLSX.LoadFromStream(AZIPStream: TStream; AReadSheets: boolean = True);
begin
  FTotCells := 0;
  FTotRows := 0;
...
begin
  ...
  if FNonStandard and (Cell.F.Content = '') then
    Cell.F.Assigneds := [];
...
begin
  ...
  if R.SpansXpgList.Count > 0 then begin
    S1 := R.SpansXpgList[0];
    S2 := SplitAtChar(AxUCChar(':'),S1);

    Row.SpanC1 := StrToIntDef(S2,$FFFF);
    Row.SpanC2 := StrToIntDef(S1,$FFFF);
  end;
  ...
  try
    if FManager.FileData.OPC.GetDrawingImages(List) then begin
      for i := 0 to List.Count - 1 do begin
        Stream := FManager.FileData.OPC.ReadDrawingMedia(AxUCString(List[i]),Target);
        if Stream <> Nil then begin
          try
...
begin
  for i := 0 to AList.Count - 1 do begin
    ...
    if D.Border.Available then
      GetBordersStyle(D.Border,DXF.AddBorder);
...
begin
  ...
  try
    ...
    try
      ...
      FNonStandard := XML.Reader.SkipNamespace;
    finally
      XML.Free;
    end;
  finally
...
begin
  ...
  for i := 0 to AList.Count - 1 do begin
    ...
    if X.NumFmtId < FManager.StyleSheet.NumFmts.Count then
...
procedure TXLSReadXLSX.ReadVBA;
//var
//  Stream: TStream;
//  FS    : TFileStream;
//  OleStorage: IStorage;
...
//  Stat: TSTATSTG;
begin
  FVBA := FManager.FileData.OPC.ReadVBA;
//  if Stream <> Nil then begin
//    FS := TFileStream.Create('d:\temp\vbaProject.bin',fmCreate);
//    try
...
//    end;
//
//    OleCheck(StgOpenStorage('d:\temp\vbaProject.bin',Nil,STGM_READ or STGM_SHARE_DENY_WRITE,Nil,0,OleStorage));
//
