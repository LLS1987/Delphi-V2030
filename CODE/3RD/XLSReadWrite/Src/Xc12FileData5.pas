...
public
     ...
     function  FindByTarget(ATarget: AxUCString): TXLSSavedFileData;
...
begin
  ...
  if i < 0 then
    raise XLSRWException.Create('Unknown OPC sheet id: ' + AId);
...
//        raise XLSRWException.CreateFmt('Unknown OPC type "%s" in save data',[List[i].Type_]);
      if SaveList.FindByTarget(List[i].Target) = Nil then begin
        if (List[i].Type_ <> OPC_XLSX_CUSTOMXML) and (List[i].Type_ <> OPC_XLSX_CUSTOMXML_PROPS) then
...
begin
  for i := 0 to FSavedRoot.Count - 1 do begin
    if Copy(FSavedRoot[i].Target,1,22) <> '../theme/themeOverride' then
...
function TXLSSavedFileDataList.FindByTarget(ATarget: AxUCString): TXLSSavedFileData;
var
  i: integer;
begin
  for i := 0 to Count - 1 do begin
    if Items[i].Target = ATarget then begin
      Result := Items[i];
      Exit;
    end;
  end;
