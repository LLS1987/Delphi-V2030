...
{$ifdef DELPHI_5}
var FEnums_1: TStringList;
{$else}
var FEnums_1: THashedStringList;
{$endif}
...
begin
  FEnums_1.AddObject('sttmExternal',TObject(0));
  FEnums_1.AddObject('sttmInternal',TObject(1));
end;
...
var
  i: integer;
begin
  i := FEnums_1.IndexOf(AValue);
  if i >= 0 then
    Result := Integer(FEnums_1.Objects[i])
  else 
    Result := 0;
end;
...
{$ifdef DELPHI_5}
  FEnums_1 := TStringList.Create;
{$else}
  FEnums_1 := THashedStringList.Create;
{$endif}
  TXPGBase.AddEnums;
