...
public
     ...
     procedure Assign(Shape: TMSOShape); virtual;
     procedure MakeMacro;
...
//* ~exclude
     TShapeClientAnchor = class(TMSOShape)
private
     FVisible: boolean;
protected
...
public
     ...
     procedure Assign(Shape: TMSOShape); override;
...
//* ~exclude
     TShapeChildAnchor = class(TMSOShape)
protected
...
//* ~exclude
     TShapeGroup = class(TMSOShape {ClientAnchor})
private
     FDrawing: TEscherDrawing;
     FList: TList;
     FSPGR: TMSORecSPGR;

     function GetItems(Index: integer): TMSOShape;
public
...
//     procedure WriteToStream(Stream: TXLSStream; PBuf: PByteArray; WriteMSODRAWING: boolean); override;
     procedure Add(Shape: TMSOShape);
     procedure Delete(Index: integer);
     procedure DeleteBySpId(SpId: integer);
     function  Count: integer;

     property Items[Index: integer]: TMSOShape read GetItems; default;
...
//* ~param Shape Not used.
     TFileReadShapeEvent = procedure (Sender: TObject; Shape: TMSOShape) of object;
...
protected
     ...
     procedure ReadOBJ(Shape: TMSOShape; Stream: TXLSStream; PBuf: PByteArray);
...
procedure TMSOShape.Assign(Shape: TMSOShape);
begin
  Shape.FSpId := 0;
...
function TMSOShape.GetFlipHorizontal: boolean;
begin
  Result := (FOptions and SpOptFlipH) = SpOptFlipH;
end;

function TMSOShape.GetFlipVertical: boolean;
begin
  Result := (FOptions and SpOptFlipV) = SpOptFlipV;
end;

procedure TMSOShape.MakeMacro;
begin
...
procedure TMSOShape.SetFlipHorizontal(const Value: boolean);
begin
  FOptions := FOptions or SpOptFlipH;
end;

procedure TMSOShape.SetFlipVertical(const Value: boolean);
begin
  FOptions := FOptions or SpOptFlipV;
end;

procedure TMSOShape.SetShapeType(const Value: integer);
begin
  FShapeType := Value;
  case FShapeType of
...
function TMSOShape.Size: integer;
begin
            // SPCONTAINER
...
procedure TMSOShape.WriteToStream(Stream: TXLSStream; PBuf: PByteArray; WriteMSODRAWING: boolean);
var
  Sz: integer;
begin
  Sz := Size;
...
procedure TShapeGroup.Add(Shape: TMSOShape);
begin
  FList.Add(Shape);
end;

procedure TShapeGroup.Assign(Shape: TMSOShape);
var
  i: integer;
  Shp: TMSOShape;
begin
...
begin
  TMSOShape(FList.Items[Index]).Free;
  FList.Delete(Index);
end;
...
begin
  for i := 0 to FList.Count - 1 do begin
    if Integer(TMSOShape(FList.Items[i]).FSpId) = SpId then begin
      Delete(i);
      Exit;
    end;
  end;
...
function TShapeGroup.GetItems(Index: integer): TMSOShape;
begin
  Result := TMSOShape(FList.Items[Index]);
end;
...
procedure TEscherDrawing.ReadOBJ(Shape: TMSOShape; Stream: TXLSStream; PBuf: PByteArray);
var
  ObjType: word;
  Len: integer;
  Header: TBIFFHeader;
        ...
        end;
{
      ...
      if (Count = 71) and (Len= 32) then
        Stream.Seek(32,soFromCurrent);
    end;
    if Len <> 0 then
...
procedure SetId(Shape: TMSOShape);
var
  i,j,Id: integer;
begin
  if Shape is TShapeGroup then begin
...
procedure SetRefCount(Shape: TMSOShape);
var
  i,Id: integer;
begin
  if Shape is TShapeGroup then begin
...
// Note_JKQWNH (Search for this).
    if TMSOShape(List[i]).FInterface is TShapeOutsideMsoChart then
      Shape.FInterface := TShapeOutsideMsoChart.Create(0,FParent,FFonts);
    TMSOShape(List[i]).Assign(Shape);
...
begin
  for i := 0 to List.Count - 1 do
    FGroup.DeleteBySpId(TMSOShape(List[i]).SpId);
end;
...
procedure TShapeClientAnchor.Assign(Shape: TMSOShape);
begin
  inherited Assign(Shape);
...
procedure TShapeChildAnchor.Assign(Shape: TMSOShape);
begin
  inherited Assign(Shape);
...
procedure TShapeGroupClientAnchor.Assign(Shape: TMSOShape);
begin
  inherited Assign(Shape);
...
procedure TShapeGroupChildAnchor.Assign(Shape: TMSOShape);
begin
  inherited Assign(Shape);
  Move(FCHILDANCHOR,TShapeChildAnchor(Shape).FCHILDANCHOR,SizeOf(TMSORecCHILDANCHOR));
end;
