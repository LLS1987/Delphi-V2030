...
#define MyAppLoggedName "单据转换"

[Setup]
; NOTE: The value of AppId uniquely identifies this application.
; Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId={{253FBD12-740E-4C28-B257-A382C7F864F5}}
AppName={#MyAppName}
...
DefaultDirName={sd}\Weway\QianFang\BillConvert
;DefaultDirName=D:\Weway\QianFang\WebServer
DisableDirPage=false
DisableProgramGroupPage=yes
Uninstallable=true
OutputDir=.\Output
OutputBaseFilename=千方百剂 单据转换 安装程序
...
//处理自己类特殊的包+菜单
Source: Upgrade.sql; DestDir: {app}\SQL\; 
Source: menuinfo.json; DestDir: {app}\Layout\; 
Source: PackageList.Dat; DestDir: {app}; 

[Icons]
Name: "{group}\千方百剂 单据转换\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; WorkingDir: "{app}"
Name: "{group}\千方百剂 单据转换\卸载 {#MyAppName}"; Filename: "{uninstallexe}"; IconIndex: -1
...
begin  
  ...
  begin 
    if Msgbox('安装程序检测到“千方百剂 单据转换”正在运行，请确认退出程序后再进行安装。' #13#13 '单击“是”确认已退出程序并继续安装，“否”退出安装。', mbConfirmation, MB_YESNO) = idNO then
    begin  
      Result := false; //安装程序退出
      IsRunning := 0; 
      exit; 
  ...
  begin
    if Msgbox('“千方百剂 单据转换”已安装过，是否卸载重装？' #13#13 '单击“是”卸载并重装，“否”退出安装程序。', mbConfirmation, MB_YESNO) = idYES then 
    begin
      UninstallBefore();
      Result := true; //安装程序继续  
    end  
